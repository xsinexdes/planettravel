...
public class PlanetEditorGUI {
    ...
    public static final int SLOT_ENTRADA = 41;
    public static final int SLOT_SALIDA = 42;
    public static final int SLOT_ZONA_INFO = 43;
    public static final int SLOT_ENTRADA_MODO = 44;
    public static final int SLOT_DETECCION = 36;
    ...
    public void abrir(Player jugador, PlanetData planeta) {
        ...
        // ---- ENTRADA / SALIDA (obligatorio marcarlos con el hacha) ----
        inv.setItem(SLOT_ENTRADA, item(Material.LIME_DYE, "§aZona de entrada (aterrizaje)",
                planeta.isEntradaUsaWorldBorder()
                        ? "§8(desactivado: usando WorldBorder)"
                        : (planeta.isEntradaLista()
                                ? "§a✔ Esquina A en §f" + (int) planeta.getSpawnPlanetaX() + ", "
                                        + (int) planeta.getSpawnPlanetaY() + ", " + (int) planeta.getSpawnPlanetaZ()
                                : "§c✖ Sin marcar (obligatorio)"),
                planeta.isEntradaLista() && !planeta.isEntradaUsaWorldBorder()
                        ? "§7Zona: §f" + (int) planeta.getEntradaZonaAnchoX() + " x " + (int) planeta.getEntradaZonaAnchoZ() + " §7bloques"
                        : "",
                "",
                "§eClic §7para recibir el hacha y marcarla",
                "§8Clic izq/der del hacha en el mundo",
                "§8" + planeta.getWorldName() + " = esquina A / B."));

        inv.setItem(SLOT_ENTRADA_MODO, item(Material.MAP, "§bModo de entrada",
                planeta.isEntradaUsaWorldBorder()
                        ? "§a✔ Aleatoria dentro del WorldBorder"
                        : "§7Zona marcada con el hacha (arriba)",
                planeta.isEntradaUsaWorldBorder()
                        ? "§8Usa el limite que pongas con /worldborder"
                        : "§8Ideal si el planeta usa el mundo entero",
                "",
                "§eClic §7para cambiar de modo"));

        inv.setItem(SLOT_DETECCION, item(Material.PURPLE_DYE, "§5Caja de deteccion (opcional)",
                planeta.isDeteccionUsaCaja()
                        ? (planeta.isDeteccionCajaLista()
                                ? "§a✔ Activa: §f" + (int) planeta.getDeteccionCajaAnchoX() + " x "
                                        + (int) planeta.getDeteccionCajaAnchoY() + " x " + (int) planeta.getDeteccionCajaAnchoZ()
                                : "§e⚠ Activa pero sin marcar (usa la esfera)")
                        : "§7Desactivada: usando la esfera (radio " + (int) planeta.getRadio() + ")",
                "§8Si la activas, entras por CUALQUIER",
                "§8lado de la caja, no solo por el radio.",
                "",
                "§eClic izq §7recibir el hacha y marcarla",
                "§eClic der §7activar/desactivar este modo"));

        inv.setItem(SLOT_SALIDA, item(Material.ORANGE_DYE, "§6Zona de salida (despegue)",
                planeta.isSalidaLista()
                        ? "§a✔ Esquina A en §f" + (int) planeta.getSalidaEspacioX() + ", "
                                + (int) planeta.getSalidaEspacioY() + ", " + (int) planeta.getSalidaEspacioZ()
                        : "§c✖ Sin marcar (obligatorio)",
                planeta.isSalidaLista()
                        ? "§7Zona: §f" + (int) planeta.getSalidaZonaAnchoX() + " x " + (int) planeta.getSalidaZonaAnchoY()
                                + " x " + (int) planeta.getSalidaZonaAnchoZ() + " §7bloques"
                        : "",
                "",
                "§eClic §7para recibir el hacha y marcarla",
                "§8Clic izq/der del hacha en el espacio",
                "§8= esquina A / B."));

        inv.setItem(SLOT_ZONA_INFO, item(Material.MAP, "§dComo funciona la zona",
                "§7Cada vez que alguien aterriza o",
                "§7despega, se elige un punto AL AZAR",
                "§7dentro de la zona marcada.",
                "§7Asi el planeta se siente fisico",
                "§7de verdad y no todos aparecen",
                "§7exactamente en el mismo sitio."));
