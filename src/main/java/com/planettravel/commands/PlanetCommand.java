...
public class PlanetCommand implements CommandExecutor, TabCompleter {
    ...
    private static final List<String> CAMPOS_EDITABLES = Arrays.asList(
            ...
            "entradaworldborder", "deteccioncaja", "salidamargen",
    ...
    private void entregarHachaEntrada(Player jugador, PlanetData planeta) {
        jugador.getInventory().addItem(plugin.getSelectionManager().crearHacha());
        plugin.getSelectionManager().activarModoPunto(jugador, TipoPunto.ENTRADA, planeta.getId());
        jugador.sendMessage("§b✔ Hacha entregada. §7Ve al mundo §f" + planeta.getWorldName()
                + " §7y marca la zona de ENTRADA (donde aterrizan las naves):");
    ...
    private boolean crear(CommandSender sender, String[] args) {
        ...
        sender.sendMessage("§7Siguientes pasos §c(obligatorio)§7:");
        ...
        sender.sendMessage("§7 2. Ajusta el entorno con §f/planeta gui " + id);
        sender.sendMessage("§7La salida (despegue) es automatica, no hace falta marcarla: aparece justo fuera del "
                + "radio de deteccion. Ajustable con §f/planeta editar " + id + " salidamargen <bloques>.");
        sender.sendMessage("§7El planeta no permitira viajar hasta que la entrada este lista.");
    ...
    private boolean editar(CommandSender sender, String[] args) {
        ...
        try {
            switch (campo) {
                ...
                case "salidamargen":
                    requiereArgs(args, 4, "editar " + id + " salidamargen <bloques>");
                    datos.setSalidaMargen(d(args[3]));
                    sender.sendMessage("§a✔ Margen de salida de '" + id + "' ahora es " + (int) datos.getSalidaMargen()
                            + " bloques (radio de deteccion + este margen = distancia a la que apareces al despegar).");
                    break;
    ...
    //  /planeta hacha entrada <id>           -> modo: marcar zona de entrada
    //  /planeta hacha deteccion <id>         -> modo: marcar caja de deteccion (opcional)
    //  /planeta hacha sol                    -> modo: marcar el sol del espacio
    //  (la salida/despegue ya NO se marca: es automatica a partir del radio)
    // ---------------------------------------------------------------
    private boolean hacha(CommandSender sender, String[] args) {
        ...
        if (!modo.equals("entrada") && !modo.equals("deteccion")) {
            sender.sendMessage("§cUso: /planeta hacha [entrada|deteccion <id> | sol]");
            sender.sendMessage("§7(la salida ya no se marca: es automatica, ver /planeta editar <id> salidamargen)");
            return true;
        }
        if (args.length < 3) {
        ...
        entregarHachaEntrada(jugador, datos);
        return true;
    }
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        ...
        if (args[0].equalsIgnoreCase("hacha")) {
            if (args.length == 2) {
                return filtrar(Arrays.asList("entrada", "deteccion", "sol"), args[1]);
            }
            if (args.length == 3 && (args[1].equalsIgnoreCase("entrada") || args[1].equalsIgnoreCase("deteccion"))) {
