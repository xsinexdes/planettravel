 ...
 * {@link SelectionManager.ModoPunto}) dibuja en vivo la zona de ENTRADA que
 * esta configurando (verde), la caja de DETECCION opcional (morado), o la
 * posicion del SOL (amarillo), para que vea exactamente donde va a caer
 * antes de confirmar con el hacha. La salida ya no se marca: es automatica.
 */
public class SelectionPreviewTask extends BukkitRunnable {
    ...
    private void dibujarModoPunto(Player jugador, SelectionManager.ModoPunto modo) {
        ...
        if (!esEntrada) {
            return; // no deberia llegar aqui (SOL y DETECCION ya se gestionaron arriba)
        }
        String mundoEsperado = planeta.getWorldName();
