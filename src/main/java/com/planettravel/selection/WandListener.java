 ...
 *
 * Modo "marcar punto" (activado desde /planeta hacha entrada|salida <id> o
 * desde el editor): CUALQUIER clic con el hacha en el mundo correcto fija
 * ese punto exacto (entrada al planeta, salida al espacio, o el sol). Este es
 * el UNICO sitio del plugin donde se pueden fijar esos puntos: por diseno, no
 * hay comando que los escriba a mano, asi que usar el hacha es obligatorio.
 */
public class WandListener implements Listener {
    ...
    private void manejarSeleccionPunto(Player jugador, ModoPunto modo, Location punto, boolean esClicIzquierdo) {
        ...
        if (esDeteccion) {
            ...
            if (esClicIzquierdo) {
                planeta.setSalidaEspacio(destino.getX(), destino.getY(), destino.getZ(), destino.getYaw(), destino.getPitch());
                jugador.sendMessage("§a✔ Esquina A de SALIDA de '" + planeta.getNombreVisible() + "' marcada en §f"
                        + destino.getBlockX() + ", " + destino.getBlockY() + ", " + destino.getBlockZ() + "§a.");
            } else {
                planeta.setSalidaEspacioEsquinaB(destino.getX(), destino.getY(), destino.getZ());
                jugador.sendMessage("§a✔ Esquina B de SALIDA marcada. §7Zona: §f"
                        + (int) planeta.getSalidaZonaAnchoX() + " x " + (int) planeta.getSalidaZonaAnchoY()
                        + " x " + (int) planeta.getSalidaZonaAnchoZ() + " §7bloques.");
            }
            planeta.setSalidaLista(true);
            if (planeta.salidaEstaDentroDeSuPropiaEsfera()) {
                jugador.sendMessage("§e⚠ El centro de esa zona cae DENTRO de la propia zona de deteccion del "
                        + "planeta. Eso provocara un bucle de teletransporte: marcala mas lejos.");
            }
        }
        ...
        if (planeta.puedeViajar()) {
            jugador.sendMessage("§b✔ '" + planeta.getNombreVisible() + "' ya tiene entrada y salida configuradas: listo para viajar.");
        }
    }
