...
public class TemperatureManager {
    ...
    public TemperatureManager(SuitManager suits, PlanetConfigManager configManager) {
        this.suits = suits;
    ...
    public double calcularTemperaturaAmbiente(Contexto ctx, Location loc) {
        ...
        // EL SOL: en el espacio, si hay un sol marcado con el hacha, la
        // temperatura depende de la distancia a el (cerca = calor, lejos =
        // frio) en vez del ciclo dia/noche normal, que no tiene sentido en
        // el vacio.
        SpaceSettings espacio = configManager == null ? null : configManager.getSpaceSettings();
        if (ctx.esEspacio() && espacio != null && espacio.isSolActivo()) {
            temperatura = espacio.temperaturaPorDistanciaAlSol(loc.getX(), loc.getY(), loc.getZ(), temperatura);
