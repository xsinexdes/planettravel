...
public class TeleportManager {
    ...
    public void aterrizarEnPlaneta(Player piloto, Ship nave, PlanetData planeta) {
        ...
        if (!planeta.entradaConfigurada()) {
            piloto.sendMessage("§c✖ '" + planeta.getNombreVisible() + "' todavia no tiene un punto de entrada "
                    + "marcado con el hacha de seleccion. Avisa a un admin.");
            return;
        }
        ...
        // Conservamos el yaw/pitch de vuelo actual de la nave; la posicion X/Z
        // se sortea dentro de la ZONA de entrada marcada con el hacha (o el
        // punto exacto si solo se marco la esquina A), y la Y es siempre la
        // altura de entrada atmosferica configurada para el planeta.
        Location origen = nave.getLocation();
        Location destino = planeta.isEntradaUsaWorldBorder()
                ? planeta.puntoEntradaAleatorioWorldBorder(mundoPlaneta, clampAltura(planeta.getEntradaAtmosfericaY(), mundoPlaneta))
                : planeta.puntoEntradaAleatorio(mundoPlaneta, clampAltura(planeta.getEntradaAtmosfericaY(), mundoPlaneta));
    ...
    public void despegarDePlaneta(Player piloto, Ship nave, PlanetData planeta) {
        ...
        if (!planeta.isSalidaLista()) {
            piloto.sendMessage("§c✖ '" + planeta.getNombreVisible() + "' todavia no tiene un punto de salida "
                    + "marcado con el hacha de seleccion. Avisa a un admin.");
            return;
        }
        ...
        Location origen = nave.getLocation();
        // La salida real se sortea dentro de la ZONA marcada con el hacha en
        // el espacio (esquina A/B), siempre cerca del planeta fisico.
        Location destino = planeta.puntoSalidaAleatorio(mundoEspacio);
        destino.setYaw(origen.getYaw());
        destino.setPitch(origen.getPitch());
    ...
    private void avisarPeligros(Player piloto, PlanetData planeta) {
        ...
        if (env.isRequiereCasco()) {
            String necesitas;
            if (env.getTierCasco() > 0) {
                int tier = Math.min(env.getTierCasco(), Math.max(1, configManager.getMaxTierDefinido()));
                necesitas = "un traje espacial tier " + tier + (env.isRequiereTrajeCompleto() ? " completo" : "");
            } else {
                necesitas = env.getMaterialCasco().toLowerCase().replace('_', ' ');
            }
            piloto.sendMessage("§c ✖ §7Sin atmosfera respirable. Necesitas §f" + necesitas + "§7.");
        }
        if (env.getPresion() >= configManager.getPresionPeligrosa()) {
            piloto.sendMessage(String.format("§4 ⚠ §7Presion aplastante (§f%.1f atm§7): hace falta traje completo resistente.",
                    env.getPresion()));
        }
        if (env.isMeteoritos()) {
            piloto.sendMessage("§6 ☄ §7Hay lluvia de meteoritos en este planeta.");
        }
        if (env.isAguaToxica()) {
